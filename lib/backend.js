...
var fs = require('fs'), Util = require('./helper/util'), Q = require('q'), EventEmitter2 = require('eventemitter2').EventEmitter2, Word = require('./helper/word');
...
var Backend = (function() {
  ...
  Backend.prototype.initialize = function() {
    ...
    this.emitter.on('app.game.player.moveDeclined', events.onPlayerMoveDeclined.bind(this));
  ...
  Backend.prototype.findGameByPlayerId = function(playerId) {
    ...
    for (gameId in this.games) {
      if (this.games.hasOwnProperty(gameId)) {
        var game = this.games[gameId];
        if (!game) {
          this.log('Somehow no game was found at this place.');
          return;
        }
  ...
  Backend.prototype.move = function(socket, data) {
    ...
    var word = '', i, position, playerType = this.findPlayerTypeBySocketId(game.id, socket.id), wordExists = false;
    ...
    // word already played?
    var wordPlayed = game.player1Words.indexOf(word) !== -1 || game.player2Words.indexOf(word) !== -1;
    if (!wordPlayed) {
      wordExists = Word.check(WORD_DICTIONARY, word);
    }

    var stateMatrix = '0120201202012020120201202'; // dummy

    var success = !wordPlayed; // && wordExists;
    ...
    if (success) {
      ...
      this.emit('app.game.player.moved', {gameId: game.id, success: true, socketId: socket.id, stateMatrix: stateMatrix, word: word, lastPlayer: lastPlayer});
    } else {
      this.emit('app.game.player.moveDeclined', {gameId: game.id, success: true, socketId: socket.id, stateMatrix: stateMatrix, word: word});
    }
  };
  ...
  var events = {
    ...
    onPlayerPreMoved: function(data) {
      ...
      if (playerType) {
        this.broadcastToGamePlayers(data.gameId, playerType, {action: 'playerPreMoved', positions: data.positions});
      }
    },
    onPlayerMoved: function(data) {
      var publicGame = this.getGame(data.gameId);
      this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.ALL_PLAYERS, {action: 'playerMoved', game: publicGame, word: data.word, stateMatrix: data.stateMatrix, lastPlayer: data.lastPlayer});
    },
    onPlayerMoveDeclined: function(data) {
      var publicGame = this.getGame(data.gameId);
      var playerType = this.findPlayerTypeBySocketId(data.gameId, data.socketId);
      this.broadcastToGamePlayers(data.gameId, playerType, {action: 'playerMoveDeclined'});
    },
