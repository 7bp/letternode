  ...
  Backend.prototype.broadcastToGamePlayers = function(gameId, clientType, data) {
    ...
    for (var i = 0; i < clients.length; i++) {
      ...
      try {
        ...
        this.log('error', 'The client #' + socket.id + ' seems to be disconnected. This client will be registered now.')
  ...
  Backend.prototype.dispatchMessage = function(socket, data) {
    var commands = {
      ...
      joinGame: function(socket, data) {
        ...
        this.broadcastToGamePlayers(game.id, 'player1', {action: 'gameJoined', game: resultGame});
        this.broadcastToGamePlayers(game.id, 'player2', {action: 'gameJoined', game: resultGame});
      },
