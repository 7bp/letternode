...
var fs = require('fs'), Util = require('./helper/util'), Q = require('q'), EventEmitter2 = require('eventemitter2').EventEmitter2, Words = require('./words'), MatrixUtil = require('./helper/matrix_util'), ScoreUtil = require('./helper/score_util');
...
var Backend = (function() {
  ...
  Backend.prototype.move = function(socket, data) {
    ...
    var me = this,
      ...
      wordPlayed = true,
      playerMoveValid = (game.activePlayer === 1 && playerType === CLIENT_TYPES.PLAYER_1) ||
    ...
    if (playerMoveValid) {
      ...
      if (Util.checkSearchDoesNotExistInWords(word, game.player1Words) && Util.checkSearchDoesNotExistInWords(word, game.player2Words)) {
        this.words.existWord(word).then(function() {
          ...
          game.stateMatrix = MatrixUtil.buildStateMatrixAsString(game.stateMatrix, playerType, positions);
          var score = ScoreUtil.getScores(stateMatrix);
          game.player1Score = score[0];
          game.player2Score = score[1];
          if (ScoreUtil.isGameFinished(stateMatrix)) {
        ...
        this.emit('app.game.player.moved', {gameId: game.id, success: false, socketId: socket.id, word: word, failureType: 'word_already_played'});
      }
    } else {
      this.emit('app.game.player.moved', {gameId: game.id, success: false, socketId: socket.id, word: word, failureType: 'invalid_move'});
    }
  };
