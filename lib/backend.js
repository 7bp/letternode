...
var Backend = (function() {
  ...
  Backend.prototype.deregisterClient = function(socket) {
    ...
    for (var i = 0; i < eventsToEmit.length; i++) {
  ...
  Backend.prototype.cleanClients = function(gameId, activityThreshold) {
    ...
    if (clients) {
      ...
      if (activityThreshold) {
        ...
        for (var i = 0; i < clients.length; i++) {
          var client = clients[i];
          if (client.updated < activityThreshold) {
  ...
  Backend.prototype.onClientRegistered = function(data) {
    ...
    if (game.state === GAME_STATES.INIT) {
      this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.PLAYER_1, {action: 'gameCreated', game: publicGame});
    } else {
      this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.PLAYER_1, {action: 'playerJoined', game: publicGame});
      this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.PLAYER_2, {action: 'playerJoined', game: publicGame});
    }
  };

  Backend.prototype.onClientDeRegistered = function(data) {
    var game = this.games[data.gameId], publicGame = this.getGame(game.id);
    this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.PLAYER_1, {action: 'playerLeaved', game: publicGame});
    this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.PLAYER_2, {action: 'playerLeaved', game: publicGame});
  };
