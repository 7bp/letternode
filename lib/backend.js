...
var fs = require('fs'), Util = require('./helper/util'), Q = require('q'), EventEmitter2 = require('eventemitter2').EventEmitter2, Words = require('./words');
...
var Backend = (function() {
  ...
  // Constructor
  function Backend(storagePath, backdoorEnabled) {
    this.games = {};
  ...
  Backend.prototype.broadcastToGamePlayers = function(gameId, clientType, data) {
    if (this.io) {
      ...
      if (data.action) {
        ...
        if (this.backdoorEnabled) {
          this.io.sockets.in('backdoor').emit(action, data);
        }
      } else {
        sockets.emit('message', data);
        if (this.backdoorEnabled) {
          this.io.sockets.in('backdoor').emit('message', data);
        }
      }
    }
  };
  ...
  Backend.prototype.move = function(socket, data) {
    ...
    if (playerMoveValid) {
      ...
      // word already played?
      wordPlayed = game.player1Words.indexOf(word) !== -1 || game.player2Words.indexOf(word) !== -1;
      if (!wordPlayed && game.player1Words.length > 0) {
        for (i = 0; i < game.player1Words.length; i++) {
          if (word.length <= game.player1Words[i].length && game.player1Words[i].substring(0, word.length) === word) {
            wordPlayed = true;
          }
        }
      }
      if (!wordPlayed && game.player2Words.length > 0) {
        for (i = 0; i < game.player2Words.length; i++) {
          if (word.length <= game.player2Words[i].length && game.player2Words[i].substring(0, word.length) === word) {
            wordPlayed = true;
          }
        }
      }
      if (!wordPlayed) {
        this.words.existWord(word).then(function() {
          var stateMatrix = '0120201202012020120201202'; // dummy
          var playerNum;
          ...
          game.stateMatrix = stateMatrix;
          me.syncData();
          me.emit('app.game.player.moved', {gameId: game.id, success: true, socketId: socket.id, word: word, playerNum: playerNum});
        }, function() {
          me.emit('app.game.player.moved', {gameId: game.id, success: false, socketId: socket.id, word: word, failureType: 'word_invalid'});
        });
      }
    }

    var failureType = 'invalid_move';
    if (playerMoveValid && wordPlayed) {
      failureType = 'word_already_played';
    }
    if (!playerMoveValid || wordPlayed) {
      this.emit('app.game.player.moved', {gameId: game.id, success: false, socketId: socket.id, word: word, failureType: failureType});
    }
  };
  ...
  Backend.prototype.dispatchMessage = function(socket, data) {
    var commands = {
      preMove: function(socket, data) {
        this.preMove(socket, data);
      },
      ...
      restart: function(socket, data) {
        ...
        var stateMatrix = '0000000000000000000000000';
  ...
  Backend.prototype.createGameMatrix = function() {
    ...
    for (i = 0; i < 25; i++) {
      ...
      if (vowels.indexOf(chars[position]) !== -1) {
        ic++;
      }
    }
    ...
    while (ic <= 2) {
      ...
      result[position] = vowels[Math.floor(Math.random() * vowels.length)];
    }
    return result;
  };
  ...
  Backend.prototype.registerBackdoor = function(socket) {
    if (this.backdoorEnabled) {
      ...
      for (var gameId in this.games) {
        if (this.games.hasOwnProperty(gameId)) {
          games.push(this.getGame(gameId));
        }
      }
  ...
  var events = {
    ...
    onPlayerMoved: function(data) {
      ...
      if (!data.success) {
        var playerType = this.findPlayerTypeBySocketId(data.gameId, data.socketId);
        var failureType = (data.failureType != null && data.failureType) ? data.failureType : 'invalid_move';
    ...
    onClientLeft: function(data) {
      ...
      this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.ALL_PLAYERS, {action: 'playerLeft', game: publicGame});
    }
  };
