    ...
    if (playerMoveValid) {
      ...
      if (Util.checkSearchDoesNotExistInWords(word, game.player1Words) && Util.checkSearchDoesNotExistInWords(word, game.player2Words)) {
        this.words.existWord(word).then(function() {
          ...
          game.player1List = getSocketIdsBySockets(this.io.sockets.clients(this.getGamePlayerRoomName(game.id, CLIENT_TYPES.PLAYER_1)));
          game.player2List = getSocketIdsBySockets(this.io.sockets.clients(this.getGamePlayerRoomName(game.id, CLIENT_TYPES.PLAYER_2)));
          game.updated = new Date().getTime();
      ...
      this.emit('app.game.player.moved', {gameId: game.id, success: false, socketId: socket.id, word: word, failureType: 'game_already_finished'});
    } else {
