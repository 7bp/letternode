...
var Backend = (function() {
  ...
  Backend.prototype.move = function(socket, data) {
    var game = this.games[data.gameId], word = data.word;
    if (!game) {
      this.log('info', 'Action "move" was performed without a valid game id.');
      return;
    }

    var wordExists = Word.check(WORD_DICTIONARY, 'test');
    ...
    if (success) {
      this.emit('app.game.player.moved', {gameId: game.id, success: true, socketId: socket.id, stateMatrix: stateMatrix, word: word});
    } else {
  ...
  var events = {
    ...
    onPlayerMoved: function(data) {
      var publicGame = this.getGame(data.gameId);
      this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.ALL_PLAYERS, {action: 'playerMoved', game: publicGame, word: data.word, stateMatrix: data.stateMatrix});
    },
