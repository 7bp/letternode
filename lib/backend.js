...
var fs = require('fs'), Util = require('./helper/util'), Q = require('q'), EventEmitter2 = require('eventemitter2').EventEmitter2, Word = require('./helper/word'), Words = require('./words');
...
var Backend = (function() {
  ...
  Backend.prototype.move = function(socket, data) {
    ...
    var word = '', i, position, playerType = this.findPlayerTypeBySocketId(game.id, socket.id), wordExists = false, wordPlayed = true, playerMoveValid = false;
    if (
    ...
    this.log('info', 'playerMoveValid: ' + playerMoveValid + ' game.activePlayer: ' + game.activePlayer + ', playerType: ' + playerType);
    ...
    var success = playerMoveValid && !wordPlayed; // && wordExists;
    ...
    if (success) {
      ...
      this.emit('app.game.player.moved', {gameId: game.id, success: true, socketId: socket.id, word: word, lastPlayer: lastPlayer});
    } else {
      this.emit('app.game.player.moved', {gameId: game.id, success: false, socketId: socket.id, word: word});
    }
  };
  ...
  var events = {
    ...
    onPlayerMoved: function(data) {
      ...
      if (!data.success) {
        ...
        this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.ALL_PLAYERS, {action: 'playerMoved', game: publicGame, word: data.word, lastPlayer: data.lastPlayer});
      }
    },
