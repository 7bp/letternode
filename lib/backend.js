...
var Backend = (function() {
  ...
  function Backend(config) {
    this.games = {};
    this.storagePath = __dirname + '/../' + config.backend.store_file_path;
    this.gameSpec = cloneObject(config.gameSpec);
    this.gameSpec.matrix.cells = this.gameSpec.matrix.rows * this.gameSpec.matrix.columns_per_row;
    this.emitter = new EventEmitter2({wildcard: true, maxListeners: 50});
    this.words = new Words({
      fileNameChars: config.backend.dictionary.length,
      basePath: config.backend.dictionary.path
    });
    this.syncDataBuffer = 5000;
    this.backdoorEnabled = config.backend.backdoor.enabled === true;
    this.log('info', 'Backend starting initialization...');
    if (this.backdoorEnabled) {
      this.log('warn', 'Backdoor mode was initialized!');
    }
    this.matrixUtil = new MatrixUtil(this.gameSpec.matrix);
    this.initialize();
  }
  ...
  Backend.prototype.createGame = function(socket, options) {
    ...
    game.activePlayer = 1;
    game.matrixConfig = cloneObject(this.gameSpec.matrix);
  ...
  Backend.prototype.getGame = function(gameId) {
    ...
    var result = {
      ...
      state: game.state,
      matrixConfig: game.gameSpec.matrix
    };
    return result;
  };
