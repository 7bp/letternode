...
var Backend = (function() {
  ...
  Backend.prototype.initialize = function() {
    this.emitter.on('app.game.client.joined', this.onClientJoined.bind(this));
    this.emitter.on('app.game.client.left', this.onClientLeft.bind(this));
    this.emitter.on('app.game.player.moved', this.onPlayerMoved.bind(this));
    this.emitter.on('app.game.state.changed', this.onGameStateChanged.bind(this));
  ...
  Backend.prototype.registerClient = function(gameId, socket, type) {
    ...
    this.emit('app.game.client.joined', {gameId: gameId, sockedId: socket.id, type: type});
  };
  ...
  Backend.prototype.deRegisterClient = function(socket) {
    var removeSocketIdInList = function(list, socketId) {
      for (var j = 0; j < list.length; j++) {
        if (list[j] === socketId) {
          list.splice(j, 1);
          // Break is required: (1) splicing enforce loop handling to stop, (2) the socket id should be unique
          break;
        }
      }
    };
    if (this.io) {
      ...
      for (var room in rooms) {
        if (rooms.hasOwnProperty(room)) {
          ...
          var prefix = 1 + GAME_ROOM_PREFIX.length;
          if (room.length === prefix + GAME_ID_LENGTH) {
            var gameId = room.slice(prefix.length);
  ...
  Backend.prototype.getGame = function(gameId) {
    ...
    var result = {
      ...
      player1Cnt: game.player1List.length,
      player2Cnt: game.player2List.length
    };
    return result;
  };
  ...
  Backend.prototype.onGameStateChanged = function(data) {
  ...
  Backend.prototype.onPlayerMoved = function(data) {
    var publicGame = this.getGame(data.gameId);
    this.broadcastToGamePlayers(data.gameId, CLIENT_TYPES.ALL_PLAYERS, {action: 'playerMoved', game: publicGame, move: data.move});
  };

  Backend.prototype.onClientJoined = function(data) {
  ...
  Backend.prototype.onClientLeft = function(data) {
    var game, publicGame;
